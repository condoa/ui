<template>
    <ContextMenuSubTrigger data-slot="context-menu-sub-trigger" :data-inset="inset" v-bind="forwarded" :class="styles({ inset, class: props.class })">
        <slot>
            <span v-if="title">{{ title }}</span>
        </slot>
        <Icon class="ml-auto size-4" :name="icon || 'icon-chevron-right'" aria-hidden="true" />
