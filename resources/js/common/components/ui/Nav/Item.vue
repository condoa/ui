<template>
    ...
    <div v-else v-bind="forwarded" data-slot="navbar-item" :class="styles().wrapper({ class: props.class })">
        <slot />
        <span data-navbar="current-indicator" :class="styles().indicator()" />
    </div>
</template>
...
const styles = tv({
    slots: {
        wrapper: [
            'group/sidebar-item hover:bg-current/5',
            ...
            'text-left disabled:cursor-default cursor-pointer select-none disabled:opacity-50',
        ],
        indicator: [
