<template>
    <Primitive data-slot="nav-provider" :data-state="open ? 'open' : 'closed'" v-bind="forwarded" :class="navProviderStyles({ class: props.class })">
        <slot />
    </Primitive>
</template>
...
import { type Ref, provide, watch } from 'vue'
import { navProviderKey } from './provider'

type NavProviderProps = PrimitiveProps & {
...
const props = withDefaults(defineProps<NavProviderProps>(), { as: 'div', onOpenChange: () => {} })
