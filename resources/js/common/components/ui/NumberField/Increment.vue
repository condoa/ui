<template>
    <NumberFieldIncrement data-slot="number-field-increment" aria-label="Increase value" v-bind="forwarded" :class="styles({ class: props.class })">
        <slot>
...
interface Props extends NumberFieldIncrementProps {
    class?: ClassValue
    icon?: string
}

const props = withDefaults(defineProps<Props>(), { icon: 'icon-plus' })
