<template>
    <div class="w-full">
        <Label v-if="formLabel" :for="inputId" :class="[disabled && 'text-muted-foreground', errorMessage && 'text-destructive', 'mb-2']">
            <span>{{ formLabel }} <span v-if="required" class="text-destructive">*</span></span>
        </Label>
        <Multiselect v-bind="{ ...$attrs, ...forwarded }" :id="inputId" ref="multiselect" v-model="value" :attrs="{ id: inputId, 'aria-invalid': !!errorMessage, 'aria-describedby': errorMessage ? errorId : undefined }" :name="name" :classes="{ containerActive: 'ring-[3px]! ring-ring/50! transition border-primary!', containerOpen: 'rounded-b-md', containerOpenTop: 'rounded-t-md' }">
            <template v-for="(_, slotName) in slotNames" #[slotName]="slotProps">
                <slot :name="slotName" v-bind="(slotProps ?? {})" />
            </template>
            <template #clear="{ clear }">
                <button class="mr-2 flex items-center justify-center" @click="clear">
                    <Icon name="icon-close" class="text-muted-foreground size-4"></Icon>
                </button>
            </template>
        </Multiselect>
...
import { useId, useSlots, useTemplateRef } from 'vue'
...
interface Props extends MultiselectProps, MultiselectAdvancedProps {
    /**
...
const forwarded = useForwardPropsEmits(reactiveOmit(props, ["modelValue", "formLabel", "hint", "id", "name"]), emits)

const inputId = props.id || useId()
const errorId = `${inputId}-error`

const { errorMessage, value } = useField(
    () => props.name || inputId, props.rules,
    {
...
defineSlots<MultiselectSlots & Record<string, (props: Record<string, any>) => any>>()

const slots = useSlots()

const slotNames = slots as Record<string, unknown>
</script>
